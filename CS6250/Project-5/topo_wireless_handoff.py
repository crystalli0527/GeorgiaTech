...
class Topo1(Topo):
	...
	linkConfigEth = {'bw': 10, 'delay': '1ms', 'loss': 0, 'max_queue_size': max_queue_size }
	linkConfigWifi = {'bw': 2, 'delay': '5ms', 'loss': 3, 'max_queue_size': max_queue_size }
	linkConfig3G = {'bw': 2, 'delay': '75ms', 'loss': 2, 'max_queue_size': max_queue_size }
	...
	s3 = self.addSwitch('s3')	
        ...
        # Wire sender
	#self.addLink(s1, sender, **linkConfigEth)
	#self.addLink(s2, sender, **linkConfigEth)
	#self.addLink(s3, sender, **linkConfigEth)

	self.addLink(sender, s1, port1=0, port2=uplink, **linkConfigEth)
	self.addLink(sender, s2, port1=1, port2=uplink, **linkConfigEth)
	self.addLink(sender, s3, port1=2, port2=uplink, **linkConfigEth)
	
        # Wire receiver
	self.addLink(receiver, s1, port1=0, port2=downlink, **linkConfigEth)
	self.addLink(receiver, s2, port1=1, port2=downlink, **linkConfigWifi)
	self.addLink(receiver, s3, port1=2, port2=downlink, **linkConfig3G)

	#self.addLink(s1, receiver, port1=downlink, port2=12, **linkConfigEth)
	#self.addLink(s2, receiver, port1=downlink, port2=34, **linkConfigWifi)
	#self.addLink(s3, receiver, port1=downlink, port2=56, **linkConfig3G)
...
def run_parkinglot_expt(net, n):
    ...
    sleep(10) # added at the suggestion of piazza
