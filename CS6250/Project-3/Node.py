...
# Modified Fall 2016 by Jeffrey Randow to add Weight to links
...
class Node(object):
    ...
    def get_neighbor_weight(self, neighbor_name):
        ''' This function returns the weight for a specific neighbor name '''
        for neighbor in self.links:
    ...
    def verify_neighbors(self):
        ...
        for neighbor in self.links:
            node_neighbor = self.topology.topodict[neighbor.name]
            if self.name not in node_neighbor.neighbor_names:
                raise Exception(neighbor.name + " does not have link to " + self.name)
            if neighbor.weight != node_neighbor.get_neighbor_weight(self.name):
                raise Exception(neighbor.name + " has incorrect link weight to " + self.name)
